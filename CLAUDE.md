...
- **块的左侧时间是个 `Menu`**（普通态，`blockTimeMenu`）：`合并上方空闲`/`合并下方空闲`/`结束改为现在`/`开始改为现在`，按条件显示。`timeLabeledRow(menuBlock:)` 控制——传块则时间为菜单、否则纯文字（空闲/空整点行、多选态都用纯文字）。
- **底部图标按钮（多选态、恰好选中 1 个块 `singleBlock`）**：直接图标按钮（不是三点菜单）`开始改为现在`/`结束改为现在`/`并入选中空闲`(`mergeSelected`)/`合并前面空闲`(`mergeGapBefore`)/`合并后面空闲`(`mergeGapAfter`)，用 Spacer 均匀铺开。长按进多选仍是 `selectDragGesture`。
